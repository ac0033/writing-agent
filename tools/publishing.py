...
def preview(article_path: str) -> dict:
    ...
    if bundle.get("pipeline_version") == "v2":
        import re
        reviewed = bundle.get("reviewed_text", "")
        visible = re.sub(r"<!--.*?-->", "", reviewed, flags=re.S)
        visible = re.sub(r"\n{3,}", "\n\n", visible).strip() + "\n"
        fingerprint = hashlib.sha256(json.dumps([bundle.get("shared_summary", ""), reviewed], ensure_ascii=False).encode()).hexdigest()
        if (visible.encode() != article.read_bytes()
                or any(bundle.get(key) != fingerprint for key in ("reviewed_fingerprint", "checked_fingerprint", "approved_fingerprint"))
                or any(bundle.get("review_dimensions", {}).get(key) != "pass" for key in ("intent", "facts", "reading"))):
            raise ValueError("v2原意、事实、阅读质量或确认记录与当前稿件不一致")
