...
def prepare_source(topic: str, source_text: str, topic_id: str = "") -> dict:
    """v2保存原始输入，不由外层生成作者摘要，不覆盖旧文件。"""
    topic = topic.strip()
    if not topic or "\n" in topic or len(topic) > 200:
        raise ValueError("主题需为1到200字的单行标题")
    if not isinstance(source_text, str) or not source_text.strip() or len(source_text.encode()) > config.TOPIC_MAX_BYTES:
        raise ValueError("原始材料不能为空且不能超过TOPIC_MAX_BYTES")
    identity = resolve_topic_id(topic, topic_id)
    ...
    return {**meta, "topic_file": str(path.resolve()), "idea": body,
            "topic_sha256": hashlib.sha256(body.encode()).hexdigest()}
...
def load(topic_file: str) -> dict:
    ...
    try:
        ...
        if meta.get("schema") not in {"writing-topic-v1", "writing-source-v2"}:
            raise ValueError()
    ...
    if meta["schema"] == "writing-topic-v1":
        validate(body)
    elif not body.partition("\n")[2].strip():
        raise ValueError("原始材料不能为空")
