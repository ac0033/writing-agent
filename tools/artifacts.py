...
def save_evidence_bundle(state: dict, directory: Path) -> str:
    ...
    if state.get("pipeline_version") == "v2":
        bundle.update(pipeline_version="v2", shared_summary=state.get("shared_summary", ""),
                      summary_version=state.get("summary_version"), article_version=state.get("article_version"),
                      review_dimensions=state.get("review_dimensions", {}),
                      reviewer_comments=state.get("review_comments", ""),
                      reviewed_fingerprint=state.get("reviewed_fingerprint", ""),
                      checked_fingerprint=state.get("checked_fingerprint", ""),
                      approved_fingerprint=state.get("approved_fingerprint", ""),
                      reviewed_text=state.get("polished", ""),
                      reader_review="model_review", reader_independent=False,
                      decision_log=state.get("decision_log", []),
                      prompt_loads=[entry.get("prompt_files", []) for entry in state.get("thinking_log", [])])
