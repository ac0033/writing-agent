...
def test_local_fact_requires_original_path_and_verbatim_quote(monkeypatch):
    source_path = 'D:/projects/demo/status.md'
    author = source_path + '\n已验证12个单元，剩余8个待复核。'
    claim = {'claim':'已验证12个单元','kind':'local','source_url':'',
             'source_path':source_path,'quote':'已验证12个单元','assessment':'supported'}
    audit = {'verdict':'pass','issues':[],'claims':[claim]}
    monkeypatch.setattr(graph, '_run', lambda *a: (json.dumps(audit), {}))
    state = {'topic':'测试','draft':'已验证12个单元','polished':'已验证12个单元',
             'user_idea':author,'materials':[],'review_verdict':'pass'}
    assert graph.final_check(state)['publication_ready']
    claim['quote'] = '全部20个单元通过'
    assert not graph.final_check(state)['publication_ready']
    claim['quote'] = '已验证12个单元'
    claim['source_path'] = 'D:/projects/other/status.md'
    assert not graph.final_check(state)['publication_ready']


def test_revision_feedback_survives_research_and_review(monkeypatch):
    feedback = '不要重新引入已删除的二手统计'
    monkeypatch.setattr(graph, 'interrupt', lambda payload: {'route':'content','feedback':feedback})
    update = graph.human_final({'polished':'旧稿'})
    assert update['revision_feedback'] == feedback
    ...
    def run(role, node, system, user):
        assert user.count(feedback) == 1
        calls.append(role)
        if role == 'writer' and calls.count('writer') == 1:
            return '初稿\nNEED_RESEARCH: 需要一手出处', {'node':node}
        return ('VERDICT: PASS\n通过' if role == 'reviewer' else '修订稿'), {'node':node}
    monkeypatch.setattr(graph, '_run', run)
    state = {'topic':'测试','outline':'框架','materials':[], **update}
    state.update(graph.writer(state))
    assert state['needs_research']
    state.update({'needs_research':False, 'research_request':''})
    state.update(graph.writer(state))
    state.update(graph.reviewer(state))
    graph.stylist(state)
    assert calls == ['writer','writer','reviewer','stylist']
