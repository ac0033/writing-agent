...
def test_length_terminated_stream_is_not_a_complete_result():
    chunk = _chunk(content='<scratchpad>摘要</scratchpad><result>VERDICT: PASS</result>')
    chunk.choices[0].finish_reason = 'length'
    with pytest.raises(ValueError, match='finish_reason=length'):
        llm._collect_stream('reviewer', 'fake', iter([chunk]),
